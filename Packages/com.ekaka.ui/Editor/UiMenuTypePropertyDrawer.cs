...
namespace Ui.Editor
{
    [CustomPropertyDrawer(typeof(UiMenuTypeAttribute))]
    public class UiMenuTypePropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            string[] array = _uiManager.UiReferences.UiMenuReferences.Keys.ToArray();

            if (array.Length <= 0)
            {
                EditorGUI.HelpBox(position,
                    $"{nameof(UiReferences.UiMenuReferences)} list in {nameof(UiReferences)} Null or empty",
                    MessageType.Warning);
