...
namespace Ui.Utils
{
    public class FlexibleGridLayout : LayoutGroup
    {
        ...
        [Tooltip("instead of using pixels for size, fit cell to anchor (percentage of screen size) - this makes layout more responsive")]
        ...
        [Tooltip("cell height will be calculated as a percentage of this Rect's height, if null we instead use Screen.height")]
        ...
        public override void CalculateLayoutInputHorizontal()
        {
            ...
            for (int i = 0; i < childrenCount; i++)
            {
                ...
                if (centerY)
                {
                    float spaceLeftOnColumn = parentHeight - (cellSize.y * rows) - (spacing.y * (rows - 1)) -
            ...
            SetLayoutInputForAxis(0,
                (cellSize.x * columns) + (spacing.x * (columns - 1)) + padding.left + padding.right, 0, 0);
            SetLayoutInputForAxis(0, (cellSize.y * rows) + (spacing.y * (rows - 1)) + padding.top + padding.bottom, 0,
                1);
        }
